...
The error `RETRY_STATE_MAXIMUM_ATTEMPTS_REACHED` with `object of type 'NoneType' has no len()` from `EvalWorkflowCrud.get_workflow_status` appears when the database query returns no row/`None`. The subsequent call to `len(workflow_status)` then raises a TypeError. To fix this, ensure the workflow record exists or add a guard before the assertion (for example, return an empty list when no result or check for `None` first).
